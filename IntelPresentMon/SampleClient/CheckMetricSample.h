...
int CheckMetricSample(std::unique_ptr<pmapi::Session>&& pSession, const std::string& metricName)
{
    ...
    for (auto metric : pIntrospectionRoot->GetMetrics())
    {
        ...
        for (auto key : metricEnums.GetKeys())
        {
            ...
            {
                if (metricName.compare(key.GetSymbol().c_str())) {
                    break;
                }
            }
        }
        ...
        for (auto mi : metricInfo)
        {
            ...
            std::cout << std::format("Metric Id: {}, Metric Symbol: {}, Vendor Name: {}, Vendor Device Id: {}, Is Available: {}",
                (int)metric.GetId(), metricName, device.GetName(), device.GetId(), mi.IsAvailable());
            std::cout << std::endl;
        }
    }
