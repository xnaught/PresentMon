    ...
    bool computeAvg)
{
    ...
    double gpuDuration = pmSession.QpcDeltaToUnsignedMilliSeconds(p.GPUStartTime, p.ReadyTime);

    FrameMetrics metrics;
    metrics.mCPUStart             = chain->mNextFrameCPUStart;
    metrics.mCPUBusy              = pmSession.QpcDeltaToUnsignedMilliSeconds(metrics.mCPUStart, p.PresentStartTime);
    metrics.mCPUWait              = pmSession.QpcDeltaToMilliSeconds(p.TimeInPresent);
    metrics.mGPULatency           = pmSession.QpcDeltaToUnsignedMilliSeconds(metrics.mCPUStart, p.GPUStartTime);
    ...
    metrics.mDisplayLatency       = !displayed       ? 0 : pmSession.QpcDeltaToUnsignedMilliSeconds(metrics.mCPUStart, p.ScreenTime);
    metrics.mDisplayedTime        = !displayed       ? 0 : pmSession.QpcDeltaToUnsignedMilliSeconds(p.ScreenTime, nextDisplayedPresent->ScreenTime);
    metrics.mClickToPhotonLatency = p.InputTime == 0 ? 0 : pmSession.QpcDeltaToUnsignedMilliSeconds(p.InputTime, p.ScreenTime);
    ...
    uint64_t latestQpc)
{
    ...
    for (auto& pair : gProcesses) {
        ...
        for (auto ii = processInfo->mSwapChain.begin(), ie = processInfo->mSwapChain.end(); ii != ie; ) {
            ...
            if (chain->mNextFrameCPUStart < minQpc) {
