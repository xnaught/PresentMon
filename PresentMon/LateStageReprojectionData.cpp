...
void LateStageReprojectionData::PruneDeque(std::deque<LateStageReprojectionEvent> &lsrHistory, uint32_t msTimeDiff, uint32_t maxHistLen) {
    while (!lsrHistory.empty() && (
        ...
        mpSession->QpcDeltaToMilliSeconds(lsrHistory.back().QpcTime - lsrHistory.front().QpcTime) > msTimeDiff)) {
...
double LateStageReprojectionData::ComputeHistoryTime(const std::deque<LateStageReprojectionEvent>& lsrHistory) const
{
    ...
    return 0.001 * mpSession->QpcDeltaToMilliSeconds(end - start);
}
...
double LateStageReprojectionData::ComputeFps(const std::deque<LateStageReprojectionEvent>& lsrHistory) const
{
    ...
    return count / (0.001 * mpSession->QpcDeltaToMilliSeconds(end - start));
}
...
LateStageReprojectionRuntimeStats LateStageReprojectionData::ComputeRuntimeStats() const
{
    ...
    stats.mAppSourceCpuRenderTimeInMs = mpSession->QpcDeltaToMilliSeconds(totalAppSourceCpuRenderTime);
    stats.mAppSourceReleaseToLsrAcquireInMs = mpSession->QpcDeltaToMilliSeconds(totalAppSourceReleaseToLsrAcquireTime);
...
void UpdateLsrCsv(LateStageReprojectionData& lsr, ProcessInfo* proc, LateStageReprojectionEvent& p)
{
    ...
    const double deltaMilliSeconds = lsr.mpSession->QpcDeltaToMilliSeconds(curr.QpcTime - prev.QpcTime);
    const double timeInSeconds = 0.001 * lsr.mpSession->QpcToMilliSeconds(p.QpcTime);
    ...
    if (args.mTrackDisplay) {
        ...
        if (curr.IsValidAppFrame()) {
            ...
            appPresentToLsrMilliSeconds = lsr.mpSession->QpcDeltaToMilliSeconds(curr.QpcTime - currAppPresentTime);
            ...
            if (prev.IsValidAppFrame() && (curr.GetAppProcessId() == prev.GetAppProcessId())) {
                ...
                appPresentDeltaMilliSeconds = lsr.mpSession->QpcDeltaToMilliSeconds(currAppPresentTime - prevAppPresentTime);
            }
        }
    ...
    if (args.mTrackDebug) {
        fprintf(fp, ",%.6lf,%.6lf", lsr.mpSession->QpcDeltaToMilliSeconds(curr.Source.GetReleaseFromRenderingToAcquireForPresentationTime()), lsr.mpSession->QpcDeltaToMilliSeconds(curr.GetAppCpuRenderFrameTime()));
    }
...
void UpdateConsole(std::unordered_map<uint32_t, ProcessInfo> const& activeProcesses, LateStageReprojectionData& lsr)
{
    ...
    if (lsr.HasData()) {
        ConsolePrintLn(L"");
        ConsolePrintLn(L"Windows Mixed Reality:");
...

        {
            ...
            if (args.mTrackDisplay) {
                ...
                ConsolePrintLn(L"    App - %s[%d]:",
                    processIter == activeProcesses.end() ? L"<error>" : processIter->second.mModuleName.c_str(),
                    runtimeStats.mAppProcessId);
                ConsolePrint(L"        %.2lf ms/frame (%.1lf fps, %.2lf ms CPU", 1000.0 / fps, fps, runtimeStats.mAppSourceCpuRenderTimeInMs);
            } else {
                ConsolePrintLn(L"    App:");
                ConsolePrint(L"        %.2lf ms/frame (%.1lf fps", 1000.0 / fps, fps);
            }

            ConsolePrintLn(L", %.1lf%% of Compositor frame rate)", historySize == 0 ? 0.0 : double(historySize - runtimeStats.mAppMissedFrames) / (historySize) * 100.0);

            ConsolePrintLn(L"        Missed Present: %Iu total in last %.1lf seconds (%Iu total observed)",
                runtimeStats.mAppMissedFrames,
                historyTime,
                lsr.mLifetimeAppMissedFrames);

            ConsolePrintLn(L"        Post-Present to Compositor CPU: %.2lf ms",
...

        {
            ...
            ConsolePrintLn(L"    Compositor - %s[%d]:",
                processIter == activeProcesses.end() ? L"<error>" : processIter->second.mModuleName.c_str(),
                runtimeStats.mLsrProcessId);
            ConsolePrintLn(L"        %.2lf ms/frame (%.1lf fps, %.1lf displayed fps, %.2lf ms CPU)",
                1000.0 / fps,
                fps,
                lsr.ComputeDisplayedFps(),
                runtimeStats.mLsrCpuRenderTimeInMs);

            ConsolePrintLn(L"        Missed V-Sync: %Iu consecutive, %Iu total in last %.1lf seconds (%Iu total observed)",
                runtimeStats.mLsrConsecutiveMissedFrames,
                runtimeStats.mLsrMissedFrames,
                historyTime,
                lsr.mLifetimeLsrMissedFrames);

            ConsolePrintLn(L"        Reprojection: %.2lf ms gpu preemption (%.2lf ms max) | %.2lf ms gpu execution (%.2lf ms max)",
            ...
            if (runtimeStats.mCopyExecutionInMs.GetAverage() > 0.0) {
                ConsolePrintLn(L"        Hybrid Copy: %.2lf ms gpu preemption (%.2lf ms max) | %.2lf ms gpu execution (%.2lf ms max)",
            ...
            ConsolePrintLn(L"        Gpu-End to V-Sync: %.2lf ms", runtimeStats.mGpuEndToVsyncInMs);
        }

        {
            // Latency
            ConsolePrintLn(L"    Pose Latency:");
            ConsolePrintLn(L"        App Motion-to-Mid-Photon: %.2lf ms", runtimeStats.mAppPoseLatencyInMs);
            ConsolePrintLn(L"        Compositor Motion-to-Mid-Photon: %.2lf ms (%.2lf ms to V-Sync)",
                runtimeStats.mLsrPoseLatencyInMs,
                runtimeStats.mLsrInputLatchToVsyncInMs.GetAverage());
            ConsolePrintLn(L"        V-Sync to Mid-Photon: %.2lf ms", runtimeStats.mVsyncToPhotonsMiddleInMs);
        }
